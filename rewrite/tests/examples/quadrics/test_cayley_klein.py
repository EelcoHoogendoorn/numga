...
from examples.quadrics.cayley_klein_plumbing import Point, Polarity, Pole, arccos, arccosh, mv, point
...
def test_hyperbolic_distance_along_a_diameter_is_arctanh():
    ...
    for x in (0.2, 0.5, 0.75):
        np.testing.assert_allclose(arccosh(-invariant(HYPERBOLIC, origin, point(np.array([x, 0.0])))), np.arctanh(x), atol=1e-12)
...
def test_elliptic_distance_along_a_diameter_is_arctan():
    ...
    for x in (0.2, 0.5, 2.0):
        np.testing.assert_allclose(arccos(invariant(ELLIPTIC, origin, point(np.array([x, 0.0])))), np.arctan(x), atol=1e-12)
...
def test_triangle_angle_sum_is_below_pi_hyperbolic_and_above_pi_elliptic():
    ...
    for name, C in (("hyperbolic", HYPERBOLIC), ("elliptic", ELLIPTIC)):
        Q = C.inverse()
        sums[name] = arccos(to_next.regressive(Q(to_prev)) / (to_next.regressive(Q(to_next)) * to_prev.regressive(Q(to_prev))).square_root()).sum()
...
def test_circle_quadric_contains_the_points_at_its_radius():
    ...
    for _ in range(60):
        mid = 0.5 * (lo + hi)
        lo, hi = (mid, hi) if arccosh(-invariant(C, centre, point(np.array([mid, 0.25])))) < R else (lo, mid)
