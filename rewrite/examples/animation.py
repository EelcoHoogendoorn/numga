...
def save_gif(frames: list[np.ndarray], path: str, duration_ms: int, scale: float = 1.0, colors: int = 64) -> str:
    ...
    palette = [image.convert("P", palette=Image.ADAPTIVE, colors=colors, dither=Image.Dither.NONE) for image in images]
    Path(path).parent.mkdir(parents=True, exist_ok=True)
    palette[0].save(path, save_all=True, append_images=palette[1:], duration=duration_ms, loop=0, optimize=True)
