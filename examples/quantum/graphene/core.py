...
each of the honeycomb's two sublattices, and the hopping mixes them through a vector d(k), the
pseudospin field. Each of the three bonds contributes the vector x turned in the plane by the
phase k . delta across that bond; a difference between the sublattices, as in boron nitride, adds
a gap along z. The Hamiltonian is a map on even multivectors, psi -> d psi z. Its eigenvalues are
plus and minus the length of d, and the pseudospin of a state, its sandwich of z, points along d
for the upper band and against it for the lower. At the corners of the Brillouin zone d vanishes
and the two bands meet in cones.
...
back turned about its starting direction. That holonomy is a rotor with scalar part cos(gamma):
the frame has turned by 2 gamma, the solid angle the curve encloses, and gamma is the Berry phase.
Around a gapless cone it is pi, and the rotor is -1.

For comparison, in matrix notation the state reads as a column of two complex amplitudes, the
Hamiltonian as the 2x2 matrix d . sigma, and the Berry phase as the phase of the product of the
overlaps of neighbouring eigenvectors around the loop.
"""
...
def pseudospin(momentum: Vector, gap: Scalar) -> Vector:
    ...
    k . delta across the bond, plus the gap along z."""
