...
The state of a spin, or of an ensemble of spins, is half of one plus a vector r, the Bloch vector:
r points along the spin, and its length is one for a single pure spin and less for a mixture. The
pseudoscalar I squares to minus one and commutes with everything.
...
by -I times its commutator with the Hamiltonian. Relaxation enters through multivectors L, each
adding L rho L~ - (L~L rho + rho L~L) / 2, with L~ the reverse of L. One is the vector x times the
state of a spin against the field, which turns that part of the state over onto the field at the
rate 1 / T1; one along z scrambles the phase. Both are built as maps by leaving
the state open. Where the generator vanishes the state is steady; since the generator keeps the
scalar part, finding that state is a linear equation with the scalar part pinned.

The evolution over a short time dt is a map as well, exp(dt L) to fourth order in dt: a sum of
powers of the generator. Composed with itself it spans two steps, then four; composed with the
pulses, whose sandwiches are maps too, it gives a whole pulse sequence as one map.

For comparison, in matrix notation the state reads as a 2x2 Hermitian density matrix, the reverse
as the Hermitian conjugate, I as the imaginary unit i, the generator as a 4x4 superoperator on the
flattened matrix, RAISE as the raising operator sigma+, and the generator's restriction to the
Bloch vector as the Bloch equations.
"""
...
def relaxation(process: Process) -> Rates:
    """The change of a state due to a relaxation process L: L rho L~ - (L~L rho + rho L~L) / 2."""
    back = process.reverse().symmetric_reverse_product()   # [] State
    return process * State * process.reverse() - 0.5 * (back * State + State * back)
...
def generator(detuning: Scalar, drive: Scalar, t1: float, t2: float) -> Rates:
    ...
    The Hamiltonian turns the spin about the axis (drive, 0, detuning) at its length. Raising at
    the rate 1 / T1 relaxes the spin toward +z; together with it, dephasing along z at the rate
    1 / T2 - 1 / (2 T1) makes the transverse part decay at 1 / T2.
    """
...
def steady(rates: Rates) -> State:
    """The state that no longer changes.
...
def evolution(rates: Rates, dt: float) -> Evolution:
    """What becomes of each state over a time dt: exp(dt L), to fourth order in dt, the powers of
    the generator by composition."""
    small = rates * dt
    term = total = State
